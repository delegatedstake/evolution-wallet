...
// Adrian (Issue - 11): New component name
export class EosLandingComponent implements OnInit {
      ...
      this.eos.loadPublicKey(this.publicEOS).then((account_data: any) => {
        account_data.foundAccounts.forEach((acc) => {
          ...
          acc['tokens'].forEach((tk) => {
            balance += EosLandingComponent.parseEOS(tk); // Adrian (Issue - 11)
          });
          // Add stake balance
          balance += EosLandingComponent.parseEOS(acc['total_resources']['cpu_weight']); // Adrian (Issue - 11)
          balance += EosLandingComponent.parseEOS(acc['total_resources']['net_weight']); // Adrian (Issue - 11)
          const accData = {
